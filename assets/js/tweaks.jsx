...
function applyTweaks(t) {
  ...
  const L = dark
    ? { bg: 0.16, sunk: 0.19, paper: 0.22, ink: 0.94, inkSoft: 0.74, inkFaint: 0.50, rule: 0.30, ruleSoft: 0.25, accent: 0.68, accentInk: 0.78 }
    : { bg: 0.975, sunk: 0.955, paper: 0.99, ink: 0.15, inkSoft: 0.28, inkFaint: 0.42, rule: 0.78, ruleSoft: 0.86, accent: 0.54, accentInk: 0.34 };
  ...
  window.dispatchEvent(new CustomEvent('asherif:theme', {
    detail: { theme: dark ? 'dark' : 'light', source: 'tweaks' },
  }));
...
function TweaksApp() {
  ...
  React.useEffect(() => {
    applyTweaks(t);
    try { localStorage.setItem('asherif-tweaks', JSON.stringify(t)); } catch (e) {}
  }, [t]);

  return (
    <window.TweaksPanel title="Tweaks">
      <window.TweakSection title="Theme">
        <window.TweakRadio value={t.theme}
          options={[
      ...
      <window.TweakSection title="Accent color">
        <window.TweakSlider label="Hue" value={t.accentHue} min={0} max={360} step={5}
          onChange={v => setT({accentHue: v})} />
        <window.TweakSlider label="Chroma" value={t.accentChroma} min={0} max={0.24} step={0.01}
          onChange={v => setT({accentChroma: v})} />
      </window.TweakSection>
      <window.TweakSection title="Paper warmth">
        <window.TweakSlider label="Hue" value={t.bgHue} min={0} max={360} step={5}
          onChange={v => setT({bgHue: v})} />
      </window.TweakSection>
      <window.TweakSection title="Typography">
        <window.TweakSelect label="Display font" value={t.displayFont}
          options={DISPLAY_FONTS.map(f => ({label: f, value: f}))}
          onChange={v => setT({displayFont: v})} />
      </window.TweakSection>
      <window.TweakSection title="Density">
        <window.TweakRadio value={t.density}
          options={[
      ...
      <window.TweakSection title="Chrome">
        <window.TweakToggle label="Show ticker" value={t.showTicker}
          onChange={v => setT({showTicker: v})} />
      </window.TweakSection>
    </window.TweaksPanel>
  );
}
...
let _initial = {...TWEAK_DEFAULTS};
try {
  const stored = localStorage.getItem('asherif-tweaks');
  if (stored) _initial = {..._initial, ...JSON.parse(stored)};
} catch (e) {}
